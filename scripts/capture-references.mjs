...
const TARGETS = [
  {
    ...
    note: "index/about pattern",
  },
  {
    slug: "morg-timeline",
    url: "https://morg.systems/78550a77",
    note: "articles list pattern",
  },
  {
    slug: "lucumr-index",
    url: "https://lucumr.pocoo.org/",
    note: "posts list pattern",
  },
  {
    slug: "rasyidanaf-notes",
    url: "https://rasyidanaf.com/notes/",
    note: "logs list pattern",
    toggleDark: true,
  },
];
...
function dump() {
  ...
  const measureCh = (el) => {
    ...
    return {
      ...
      ch: Math.round((inner / chPx) * 10) / 10,
    };
  };
  const textOf = (el) =>
  ...
  const contentRoot =
    [...contentScores.entries()].sort(
      (a, b) => b[1].score - a[1].score,
    )[0]?.[0] ?? document.body;
  out.meta.contentRoot = {
    tag: contentRoot.tagName.toLowerCase(),
    class: contentRoot.className || null,
    blocks: contentScores.get(contentRoot)?.blocks ?? 0,
  };
  ...
  for (let i = 0; i < limit; i++) {
    ...
    if (el.textContent?.trim())
      textCount.set(cs.color, (textCount.get(cs.color) ?? 0) + 1);
    if (cs.backgroundColor !== "rgba(0, 0, 0, 0)")
      bgCount.set(
        cs.backgroundColor,
        (bgCount.get(cs.backgroundColor) ?? 0) + 1,
      );
  }
  ...
  out.colors.text = [...textCount.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 12)
    .map(([color, count]) => ({ color, count }));
  out.colors.backgrounds = [...bgCount.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 12)
    .map(([color, count]) => ({ color, count }));
  ...
  for (const el of document.querySelectorAll(
    "ul, ol, div, section, article, main",
  )) {
    if (chrome(el) || el === contentRoot) continue;
    const kids = [...el.children];
    const withLink = kids.filter(
      (k) =>
        k.querySelector("a[href]") && (k.textContent ?? "").trim().length > 3,
    );
  ...
  rowCandidates.push(
    ...[...document.querySelectorAll("main > *, article > *")]
      .map((el) => ({
        el: el.parentElement,
        kids: [...el.parentElement.children].filter(
          (k) =>
            k.querySelector("a[href]") &&
            (k.textContent ?? "").trim().length > 3,
        ),
      }))
      .filter((c) => c.kids.length >= 4),
  );
  const listPick = rowCandidates.sort(
  ...
  if (listPick) {
    ...
    const dateEl =
      row.querySelector("time, .date, [class*=date], [class*=meta]") ??
    ...
    out.rows = {
      listContainer: {
        tag: listPick.el.tagName.toLowerCase(),
        class: listPick.el.className || null,
        rows: listPick.kids.length,
        box: box(listPick.el),
      },
      ...
      titleElement: (() => {
        ...
        return {
          ...
          textDecorationLine: cs.textDecorationLine,
          description:
            after && !after.querySelector("a[href]")
              ? {
                  text: textOf(after),
                  fontSize: style(after).fontSize,
                  color: style(after).color,
                  fontFamily: style(after).fontFamily,
                }
              : null,
        };
      })(),
  ...
  for (const el of [...document.querySelectorAll("hr, [role=separator]")].slice(
    0,
    5,
  )) {
    const cs = style(el);
  ...
  for (const a of [...document.querySelectorAll("header a, nav a")].slice(
    0,
    12,
  )) {
    const cs = style(a);
  ...
  const blocks = [
    ...document.querySelectorAll(
      "main p, article p, main li, article li, main h2, article h2, main h1, article h1",
    ),
  ].slice(0, 14);
...
async function ensureBrowser() {
  try {
    ...
    // no browser on CDP_HTTP yet; the spawn below is the fallback
  }
  if (!existsSync(CHROME))
    throw new Error(`no browser on ${CDP_HTTP} and no chrome at ${CHROME}`);
  spawn(
    CHROME,
    [
      "--headless=new",
...
for (const target of TARGETS) {
  ...
  for (const vp of VIEWPORTS) {
    ...
    if (target.toggleDark) {
      await cdp
        .send(
          "Runtime.evaluate",
          {
            expression: `(() => { const toggle = [...document.querySelectorAll("a, button")].find((el) => /^\\[?dark\\]?$/i.test((el.textContent ?? "").trim())); if (toggle) { toggle.click(); return "clicked"; } return "no toggle"; })()`,
            returnByValue: true,
          },
          sessionId,
        )
        .then((r) =>
          console.log(`  ${target.slug}: dark toggle -> ${r.result?.value}`),
        );
      await new Promise((r) => setTimeout(r, 800));
    }
    const dumpResult = await cdp.send(
      "Runtime.evaluate",
      {
        expression: `(${dump.toString()})()`,
        returnByValue: true,
        awaitPromise: true,
      },
      sessionId,
    ...
    await writeFile(
      path.join(dir, `${vp.name}.json`),
      JSON.stringify(data, null, 2),
    );
    entry.viewports[vp.name] = {
      screenshot: shotPath,
      tokens: path.join(dir, `${vp.name}.json`),
      meta: data?.meta ?? null,
    };
