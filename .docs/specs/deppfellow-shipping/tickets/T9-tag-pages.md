...
- Tag normalization is one shared helper consumed by every emitter: `TagChip.astro`, `tagPath` in `src/lib/urls.ts`, and the inline-tag rewrite in `src/lib/resolve.ts`. Every `/tags/` href emitted by the build (chips, inline tags, sitemap.xml) resolves to a generated route. The generated route set is the union of front-matter tags and inline `#tags`, and the sitemap's tag locs enumerate that same union via one shared set helper (T11's completeness line: every generated public route).
...
| Tag                                                     | Page                                                |
| ------------------------------------------------------- | --------------------------------------------------- |
| `agents` on 9 notes across Articles, Projects, and Logs | `/tags/agents/` with 9 rows, category labels shown  |
...
- L-AC-06 — The sitemap's tag locs are exactly the generated `/tags/` routes, and every sitemap loc resolves to a built route. (REQ-19, REQ-23)
